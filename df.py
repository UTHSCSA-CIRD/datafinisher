...
def main(cnx,fname,style,dtcp,mincnt):
    ...
    # Why is it I sanitize them anyway instead of just using the plain old CCDs? I guess column names used to get made from these?
    # logged_execute(cnx, """update df_codeid_tmp set cpath = grsub('[^A-Za-z0-9_]','_',ccd) where ddomain not in
    logged_execute(cnx, """update df_codeid_tmp set cpath = ccd where ddomain not in ('ICD9','LOINC','NAACCR','SEER_SITE','ICD10','DEM|ETHNICITY','DEM|MARITAL','DEM|SEX','KUMC|FAMILYHISTORYDIAG','DEM|RACE','DEM|LANGUAGE') and cpath != grs('[^A-Za-z0-9_-]',cpath)""")
    # DONE: we need to find those and replace them with a sanitized version of the raw concept codes via grsub()
