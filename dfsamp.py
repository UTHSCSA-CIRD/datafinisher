...
parser.add_argument("-o","--outfile",help="SQLite output file. By default it will be the input file with 'sample_' prepended ",default="")
...
def main(cnx,samples,fromdate,todate,sel):
  ...
  for ii in ['patient_dimension','observation_fact'] :
    logged_execute(cnx,'BEGIN TRANSACTION')
    logged_execute(cnx,"delete from {0}".format(ii))
    logged_execute(cnx,"insert into {0} select * from {0}_tmp".format(ii))
    logged_execute(cnx,"drop table {0}_tmp".format(ii))
    cnx.commit()
