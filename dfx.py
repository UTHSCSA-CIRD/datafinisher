...
#if path.isfile('testinput_big.py'):
#from testinput import testheader,testmeta,testqb #,testUserRule

if(test_localdb):
  dfmdb = DFMeta('exampleinput.db',suggestions=autosuggestor)

dfm = DFMeta('exampleinput.csv',suggestions=autosuggestor)

if(test_cmpdfms):
  # comparison of two different methods of dfmeta creation
  if path.isfile('../www/demodata.csv'):
    dfm0 = DFMeta('../www/demodata.csv',suggestions=autosuggestor)
  if path.isfile('../www/demodata.db'):
    dfm1 = DFMeta('../www/demodata.db',suggestions=autosuggestor)
